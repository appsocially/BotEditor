...
var exportScenarioAsECA = function (scenarioArray) {
  ...
  var startPointEvent = entity.getContent(
    scenarioArray,
    `start-point-${projectId}`
  );
  ...
  while (true) {
    if (
      currentEvent.type == "selection" ||
      currentEvent.type == "openquestion" ||
      !currentEvent.next
    ) {
      bodyEventOfFirst += "\n";
      break;
    ...
    if (currentEvent.type == "goto")
      currentEvent = entity.getContent(scenarioArray, currentEvent.toId);
  }
  ...
  */
  ...
  for (var i = 0; i < allSelectionNodes.length; i++) {
    ...
    for (var j = 0; j < selections.length; j++) {
      ...
      if (j == 0) {
        selectionsRule += `  IF response_match(\"${
          allSelectionNodes[i].id
          }_action${j + 1}\") `;
      } else {
        selectionsRule += `  ELIF response_match(\"${
          allSelectionNodes[i].id
          }_action${j + 1}\") `;
      }
    ...
    if (selectionsRule != "") {
      resultText +=
        headerOfSelectionRule + selectionsRule + footerOfSelectionRule;
      resultText += "\n";
    }
  } // for_i
  ...
  */
  ...
  for (var i = 0; i < allOpenQuestionNodes.length; i++) {
    ...
    resultText +=
      headerOfSelectionRule + openquestionRule + footerOfSelectionRule;
    resultText += "\n";
  }
