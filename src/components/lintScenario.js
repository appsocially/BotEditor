  ...
  for (const node of scenario) {
    ...
    switch (type) {
      ...
      default:
        result = {
          warnings: [],
          errors: [error(num, `Unknown type: ${type}`)]
        };
    }
  ...
  for (const condition of conditions) {
    ...
    if (type === "custom_var" && option === undefined) {
      ...
      result.errors.push(
        error(
          num,
          `UPIL export needs edge Start -> ${
            toNode.num
          } to have a custom variable. Add one.`
        )
      );
...
const lintNormal = (node, scenario) => {
  ...
  if (customAction !== undefined) {
    result.warnings.push(
      warning(num, `UPIL export does not support Custom Action. Ignored.`)
    );
  }
...
const lintOpenQuestion = (node, scenario) => {
  ...
  if (customVariable === undefined) {
    result.errors.push(
      error(
        num,
        `UPIL export needs Open Question to have a custom variable. Add one.`
      )
    );
    ...
    result.errors.push(
      error(
        num,
        `Open Question's custom variable is missing a location. Add one.`
      )
    );
  }
  if (customAction !== undefined) {
    result.warnings.push(
      warning(node.num, `UPIL export does not support Custom Action. Ignored.`)
    );
  }
...
const lintSelection = (node, scenario) => {
  ...
  const { num, selections, customVariable, customAction } = node;

  for (const selection of selections) {
    const { label, conditions } = selection;
    if (conditions === undefined) {
      result.errors.push(
        error(
          num,
          `'${label}' selection does not have the next step. Export UPIL needs all selection to have a next step.`
        )
      );
    }
  }
  ...
  if (customVariable === undefined) {
    result.errors.push(
      error(
        num,
        `UPIL export needs Selection to have a custom variable. Add one.`
      )
    );
  } else if (customVariable.location === "") {
    result.errors.push(
      error(num, `Selection's custom variable is missing a location. Add one.`)
    );
  }
  if (customAction !== undefined) {
    result.warnings.push(
